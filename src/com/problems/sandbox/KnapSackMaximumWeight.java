...
public class KnapSackMaximumWeight {
    ...
    public static int findMaximumProfit_Recursive(List<com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item> items, int weight) {
        if(items == null || items.size() == 0) return 0;
        return findMaximumProfit_Recursive(items, items.size() - 1, weight);
    }

    private static int findMaximumProfit_Recursive(List<com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item> items, int i, int w) {
        if(i == 0) return 0;
        if(w == 0) return 0;
    ...
    public static int findMaximumProfit_TopDownDP(List<com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item> items, int weight) {
        if (items == null || items.size() == 0) return 0;
    ...
    private static int findMaximumProfit_TopDownDP(List<com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item> items, int i, int j, int[][] table) {
        if (i == 0 || j == 0) return 0;
        //if (j == 0) return 0;
        if (table[i][j] == -1) {
    ...
    public static int findMaximumProfit_BottomUpDP(List<com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item> items, int weight) {
        if(items == null || items.size() == 0) return 0;
        int[][] table = new int[items.size() + 1][weight + 1];
    ...
    public static int findMaximumProfit_BottomUpDP_WithMinSpace(List<com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item> items, int weight) {
        if(items == null || items.size() == 0) return 0;
        int[] table = new int [weight + 1];
        for(int i = 1; i <= items.size(); i++) {
            for(int j = weight; j >= 0; j--) { // this order of for loop iteration is important, because you cannot take multiple instances of an item. With unbounded, you can start from j = 1 to wgt
                int itemWeight = items.get(i-1).weight;
                int itemValue = items.get(i-1).value; // not needed
                if(itemWeight > j) continue;
                else table[j] = Math.max(table[j], itemWeight + table[j - itemWeight]);
                }
            }
        return table[weight];
    }
    ...
    public static void main(String[] args) {
        int[][] items = {{60, 5}, {50, 3}, {70, 4}, {30, 2}};
        int[][] items2 = {{21, 2}, {31, 3}, {51, 5}, {71, 7}};
        ...
        int weight = 5;
        List<com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item> itemsList = new ArrayList<>();
        for(int i = 0; i < items2.length; i++) {
            itemsList.add(new com.problems.classic_problems.dynamic_programming.KnapsackProblem.Item(items2[i][1], items2[i][0]));
        }
        System.out.print(findMaximumProfit_BottomUpDP_WithMinSpace(itemsList, 12));
