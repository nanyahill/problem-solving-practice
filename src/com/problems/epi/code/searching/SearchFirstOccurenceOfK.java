 ...
 * 2. First occurence of a search value (Think there may be duplicates)
 * 3. If value is found, it could be the first occurence or an earlier occurence may be found on the left side.
...
public class SearchFirstOccurenceOfK {
    ...
    public static int searchFirstOccurenceOfK(int[] arr, int val) {
        ...
        while (lo <= hi) {
            ...
            if (arr[mid] == val) {
                firstOccurence = mid;
                // Nothing to the right can be a first occurence
                hi = mid - 1;
