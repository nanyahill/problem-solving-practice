    ...
    for (int left = 0, right = 0; right < paragraph.size(); ++right) {
      ...
      while (remainingToCover == 0) {
        if ((result.start == -1 && result.end == -1)
          ...
          result.start = left;
          result.end = right;
          System.out.println("res = " + result.start + " " + result.end);
        }
...
//    assert(res2Array[0] == start && res2Array[1] == finish);
  ...
  private static void simpleTest() throws ClassNotFoundException, IOException {
    List<String> A = Arrays.asList("app", "ban", "app", "app", "dog", "cat", "app", "dog", "ban",
                                   "app", "cat", "dog");
    List<String> dict = Arrays.asList("ban", "cat");
...
//    simpleTestCase(A, dict, 0, 1);
//    Subarray res = findSmallestSubarrayCoveringSet(A, new HashSet<String>(dict));
    Subarray res = findSmallestSubarrayCoveringSubset(A.iterator(), dict);
